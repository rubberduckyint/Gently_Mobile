 ...
 */
export async function completeAppleSignIn(): Promise<{
  success: boolean;
  error?: string;
}> {
  try {
    ...
    const backendResult = await authenticateWithBetterAuth(
    ...
    return {
      success: false,
      error:
        error instanceof Error ? error.message : "Apple Sign In flow failed",
    };
  }
}
