...
function getAppleConfig(): AppleConfig {
  if (!appleConfig) {
    throw new Error(
      "Apple configuration has not been set. Call setAppleConfig() first.",
    );
  }
  return appleConfig;
}
...
export function getApplePrivateKey(): string {
  ...
  if (!privateKey.includes("-----BEGIN") || !privateKey.includes("-----END")) {
    throw new Error(
      "Apple private key does not appear to be in valid PEM format",
    );
  }
...
export function getAppleClientSecretInfo(): {
  ...
  const daysRemaining = Math.floor(
    (cachedToken.expiresAt - now) / (24 * 60 * 60),
  );
